      ...
      {categories.map((cat) => (
        <Tooltip key={cat.label}>
          <TooltipTrigger asChild>
            <div
              className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded border cursor-default ${cat.active ? 'border-primary/50 bg-primary/10 text-primary' : 'border-foreground/[0.08] bg-transparent text-foreground/20'}`}
            >
              {cat.icon}
