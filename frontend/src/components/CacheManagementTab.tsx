      ...
      <SettingsCard>
        <div className="flex flex-col gap-4">
          <div className="flex items-center justify-between">
            ...
            {stats && (
              <span
                className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold ${stats.backend === 'redis' ? 'bg-primary/10 text-primary border border-primary/20' : 'bg-muted text-muted-foreground border border-border'}`}
              >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
            {[
              ...
              { value: `${sizeMb.toFixed(1)} MB`, label: t('cache.memoryUsed') },
              {
                ...
                sub: t('cache.totalRequests', { count: totalRequests }),
              },
              {
                value: stats ? formatTtl(stats.cache_ttl_minutes) : '2 hours',
                label: t('cache.cacheTtl'),
              },
            ].map((item) => (
      ...
      <SettingsCard>
        <div className="flex flex-col gap-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
              <Label htmlFor="cache-ttl" className="text-xs font-semibold mb-1.5 block">
                {t('cache.ttlLabel')}
              </Label>
              <Input
                id="cache-ttl"
              ...
              <p className="text-xs text-muted-foreground mt-1">
                {t('cache.ttlHelperText', { current: formatTtl(ttlMinutes) })}
              </p>
            </div>
            <div>
              <Label htmlFor="cache-max-size" className="text-xs font-semibold mb-1.5 block">
                {t('cache.maxSizeLabel')}
              </Label>
              <Input
                id="cache-max-size"
                type="number"
              ...
              />
              <p className="text-xs text-muted-foreground mt-1">
                {t('cache.maxSizeHelperText', { current: (maxSizeMb / 1024).toFixed(2) })}
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3 items-start">
            <div className="flex-1">
              <Label htmlFor="cache-redis-url" className="text-xs font-semibold mb-1.5 block">
                {t('cache.redisUrlLabel')}
              </Label>
              <Input
                id="cache-redis-url"
                value={redisUrl}
