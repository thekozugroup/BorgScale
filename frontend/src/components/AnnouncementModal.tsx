...
export default function AnnouncementModal({
  ...
  // Keep Dialog always mounted so Radix receives open→false before content unmounts.
  // If we early-returned null while open=true, body pointer-events:none would persist.
  // IMPORTANT: DialogContent must also always be mounted (not conditionally rendered)
  // so the Radix overlay div is managed through its own animation lifecycle. Removing
  // DialogContent from the tree while the dialog is animating closed leaves the
  // overlay div orphaned in the DOM with data-state="open", which intercepts all
  // pointer events and makes sidebar navigation unclickable until a full page reload.
  const realOpen = open && !!announcement

  // Capture announcement at open time so content is available during close animation.
  // We use the last non-null announcement value so DialogContent can animate out
  // gracefully even after announcement becomes null.
  const tone = announcement ? getAnnouncementTone(announcement.type) : 'success'
  const icon = announcement ? getAnnouncementIcon(announcement.type) : null
